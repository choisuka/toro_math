...
BODY_INSERT = (
    ...
    "    // scope를 'index_pwa'로 시작하는 주소만으로 좁힌다.\n"
    "    // scope를 안 정해주면 sw.js가 있는 폴더(= /toro_math/) 전체가 범위가 되어서,\n"
    "    // 등록 코드가 없는 index.html(원본, 라이브 홈)까지 이 서비스워커의 통제를 받게 된다.\n"
    "    navigator.serviceWorker.register('./sw.js', { scope: './index_pwa' }).catch(() => {});\n"
    "  });\n"
    "}\n"
    "</script>\n"
)
